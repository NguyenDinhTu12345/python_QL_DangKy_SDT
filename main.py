...
def menu():
    ...
    print("|\033[1;34m 6: Xem dữ liệu thô đã xóa " + " "*33 + "\033[0m|")
    ...
    while True:
        try:
            ...
            if lua_chon == 1:
                ...
                print("\n")
                print("-"*160)
                print("\033[1;37mKết thúc chương trình. Tạm biệt và hẹn gặp lại!\033[0m\n") 
                print("\n")
                exit()
            else:
...
def edit_record():
    ...
    for record in data:
        if record[0] == phone_number:
            ...
            choice = input("Bạn có chắc chắn sửa thông tin của số điện thoại này không? (Nhập 1 để tiếp tục, 2 để nhập lại số điện thoại, 0 để quay về màn hình chính): ")
            if choice == '1':
                ...
                print("\033[1;31mLựa chọn không hợp lệ. Vui lòng nhập lại...\033[0m")
                return
...
def delete_record():
    ...
    for record in data:
        if record[0] == phone_number:
            choice = input("Bạn muốn xóa thông tin này vĩnh viễn (nhấn 1) hay tạm thời (nhấn 2)?. Nhấn 0 để quay lại màn hình bắt đầu: ")
            if choice == '1':
                data.remove(record)
                write_data(data)
...
def view_by_prefix():
    ...
    for record in data:
        phone_number_prefix = record[0][:3]  # Lấy 4 số đầu của số điện thoại
...
def view_raw_data_delete():
    def read_data_delete():
            data = []
            try:
                with open("Delete_DuLieu.txt", "r", encoding="UTF-8") as file:
                    for line in file:
                        data.append(line.strip().split(","))
            except FileNotFoundError:
                pass
            return data
    def write_data_delete(data):
        with open("Delete_Dulieu.txt", "w") as file:
            for record in data:
                file.write(" , ".join(record) + "\n")
    def delete_record_delete():
        data = read_data_delete()
        phone_number = input("Nhập số điện thoại yêu cầu: ")
        for record in data:
            if record[0] == phone_number:
                choice = input("Bạn muốn xóa thông tin này vĩnh viễn (nhấn 1) hay khôi phục lại thông tin đăng kí SDT(nhấn 2)?. Nhấn 0 để quay lại màn hình bắt đầu: ")
                if choice == '1':
                    data.remove(record)
                    write_data_delete(data)
                    ...
                    print("\n")
                    print("Đã xoá đăng ký số điện thoại vĩnh viễn.")
                    Iscontinue(delete_record_delete)
                    return
                elif choice == '2':
                    deleted_record = None  # Biến để lưu trữ thông tin của bản ghi bị xoá
                    ...
                    if deleted_record:
                        # Ghi thông tin của bản ghi đã xoá vào tệp Delete_DuLieu.txt
                        with open("DuLieu.txt", "a") as delete_file:
                            delete_file.write(",".join(deleted_record) + "\n")

                        # Ghi lại dữ liệu còn lại vào tệp DuLieu.txt
                        write_data_delete(new_data)

                        print("\033[1mĐã khôi phục SDT đăng kí. Thông tin được lưu ở file DuLieu.txt).\033[0m")
    ...
    data = read_data_delete()
    print("\n\033[1mDữ liệu thô trong file_delete:\033[0m")
    print("-"*160)
    for record in data:
        phone_number = record[0]
        name = record[1]
